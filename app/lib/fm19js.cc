...
void playerDatatable(const FunctionCallbackInfo<Value>& args) {
 ...
 Local<Object> playerDetail(Isolate* isolate, PlayerDetail player){
     ...
     //Technical
     technical->Set(String::NewFromUtf8(isolate, "getCorners"),  Integer::New(isolate, player.attributes.technical.getCorners ));
...
void player(const FunctionCallbackInfo<Value>& args) {
    ...
    args.GetReturnValue().Set(playerDetail(isolate,player));

}



void playerIDList(const FunctionCallbackInfo<Value>& args) {
    ...
    for(int i=0; i<sizeof(playersDetail); i++){
        ...
        data->Set(i++, playerDetail(isolate,playersDetail[i]));
